...
const Apartments = (_) => {
  const apartments = useStoreState((state) => state.apartments.list);
  const cua = useStoreState((state) => state.apartments.current);

  const updateApartments = useStoreActions(
    (actions) => actions.apartments.updateApartments
  );
    ...
    <div className="container mx-auto md:h-screen">
      <div className="flex-1">
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div
            className={`py-5 border-b-2 border-gray ${
              cua === 2 ? "border-black" : "border-gray"
            }`}
          >
            <h3
              onClick={(e) => getApartments(3, 2)}
              className={`text-center leading-none cursor-pointer ${
                cua === 2 && "tab-apartments-active"
              }`}
            >
